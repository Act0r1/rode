...
pub(crate) const ACTION_REGISTRY: &[ActionDescriptor] = &[
    ...
    action(
        ActionId::NewThreadBackspace,
        "Delete backward",
        "backspace",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadDelete,
        "Delete forward",
        "delete",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadLeft,
        "Move left",
        "left",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadRight,
        "Move right",
        "right",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadHome,
        "Move to line start",
        "home",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadEnd,
        "Move to line end",
        "end",
        Some("NewThread"),
    ),
    action(
        ActionId::NewThreadSubmit,
        "Create thread",
        "enter",
        Some("NewThread"),
    ),
...
fn binding(action: &ActionDescriptor) -> KeyBinding {
    match action.id {
        ...
        ActionId::NewThreadBackspace => KeyBinding::new(action.shortcut, Backspace, action.context),
        ActionId::NewThreadDelete => KeyBinding::new(action.shortcut, Delete, action.context),
        ActionId::NewThreadLeft => KeyBinding::new(action.shortcut, Left, action.context),
        ActionId::NewThreadRight => KeyBinding::new(action.shortcut, Right, action.context),
        ActionId::NewThreadHome => KeyBinding::new(action.shortcut, Home, action.context),
        ActionId::NewThreadEnd => KeyBinding::new(action.shortcut, End, action.context),
        ActionId::NewThreadSubmit => {
            KeyBinding::new(action.shortcut, SubmitNewThread, action.context)
        }
